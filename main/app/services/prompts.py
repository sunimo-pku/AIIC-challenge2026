...
【严格约束 —— 必须遵守】
- **直接输出纯净的面试攻略报告正文**，不要包含任何搜索/检索过程的中间叙述（例如"让我再搜索一些...""基于搜索到的信息..."等）。
- **不要输出任何邀请用户后续追问的引导语或结束语**（例如"如有任何考点需要展开深挖...可以直接问我""希望这份报告对你有帮助"等）。
- 输出内容只允许包含：报告标题、正文分析、末尾 JSON 代码块。除此之外不要有任何额外文字。
