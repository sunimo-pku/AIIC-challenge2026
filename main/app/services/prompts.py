...
支持的占位符（通过 `render_prompt` 替换）：
  ...
  历史兼容：  {prev_weaknesses} {prev_scores} {all_scores_summary}
            （旧 7 关版本残留 / 已不再注入；保留 default 防御老 chat history 中的旧 system prompt 渲染报错）
...
def render_prompt(template: str, context: dict) -> str:
    ...
    placeholders = {
        ...
        # Stage 2/3/4 全文注入（信息尽量传到底）
        "intel_report_full": context.get("intel_report_full") or "（暂无面试攻略全文）",
        "resume_text": context.get("resume_text") or "（暂无简历全文）",
        "resume_eval_full": context.get("resume_eval_full") or "（暂无简历评估全文）",
