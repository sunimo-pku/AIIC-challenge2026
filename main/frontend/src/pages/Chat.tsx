...
export default function Chat() {
  ...
  // 角色预设
  const ROLES = [
    { label: "技术面试", prompt: "你是一位资深大厂技术面试官，拥有10年以上后端/前端/算法面试经验。你的任务是：\n1. 根据用户的自我介绍或简历内容，提出有针对性的技术问题（从基础到进阶，层层深入）；\n2. 对用户的回答进行追问，模拟真实面试中的深挖场景；\n3. 每次对话结束时，给出具体的反馈：回答的优点、不足、以及更优的答法；\n4. 使用中文，语气专业但不压迫，像一位愿意帮助候选人成长的面试官。\n禁止直接给标准答案，先让用户尝试回答，再引导。" },
    { label: "行为面试", prompt: "你是一位擅长行为面试（Behavioral Interview）的HR总监，精通宝洁八大问、STAR法则等面试方法论。你的任务是：\n1. 提出经典的行为面试问题（如：请举例说明你如何解决冲突、如何领导团队、如何应对压力等）；\n2. 根据用户的回答，用STAR法则评估其结构化程度；\n3. 指出用户回答中的亮点和可以改进的地方，并给出更具体的表达建议；\n4. 帮助用户把经历包装得更有说服力和数据支撑。\n语气鼓励为主，反馈具体可执行。" },
    { label: "压力面试", prompt: "你是一位以严厉著称的压力面试官，擅长在高压场景下考察候选人的心理素质和应变能力。你的任务是：\n1. 提出尖锐、挑战性的问题（如：你最大的失败是什么？如果我发现你在简历上夸大了怎么办？）；\n2. 对用户的回答进行质疑和追问，不轻易放过模糊表述；\n3. 观察用户的情绪管理和逻辑自洽能力；\n4. 在对话结束时，给出压力面试的应对策略总结。\n注意：你的\"严厉\"是角色扮演，目的是帮助用户提前适应高压场景，最后的反馈要回归到建设性建议。" },
    { label: "通用助手", prompt: "" },
    { label: "代码专家", prompt: "你是一位资深全栈工程师，擅长给出简洁、可运行、带注释的代码。优先使用现代最佳实践。在技术面试场景中，你能帮助用户理解算法题的最优解、手写代码的边界条件、以及系统设计的关键权衡。" },
    { label: "简历诊断", prompt: "你是一位大厂HR和猎头出身的简历诊断专家。你的任务是：\n1. 分析用户提供的简历内容，找出亮点和不足；\n2. 判断简历是否与目标岗位匹配，给出修改建议；\n3. 帮助用户用数据化和成果导向的方式重新表达经历；\n4. 预测面试官可能针对这份简历提出的问题，让用户提前准备。\n反馈要直接、具体、可操作，不要泛泛而谈。" },
  ];
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw) {
        setCustomInstructions(JSON.parse(raw));
      } else {
              ...
              <div className="flex items-center gap-3">
                {isStreaming ? (
                  <span className="flex items-center gap-2">
                    <span className="text-accent text-[11px]">生成中</span>
                ...
                <span className="text-[11px] text-fg-subtle">Δ {latency}</span>
              </div>
            </div>
            <div
              ref={scrollRef}
