      ...
      <div className="h-full grid grid-cols-1 lg:grid-cols-[1fr_420px]">
        <section className="border-r border-border p-6 overflow-y-auto space-y-6">
          ...
          <div className="space-y-4">
            {fields.map((f) => (
              <div key={f.key} className="space-y-1">
                <label className="block text-[12px] text-fg-muted uppercase tracking-[0.12em] font-mono">{f.label}</label>
                <textarea value={entries[0][f.key]} onChange={(e) => updateField(f.key, e.target.value)}
          ...
          <button onClick={handleSubmit}
            ...
            {loading ? <Loader2 size={14} className="animate-spin" /> : <>AI 点评 <ArrowRight size={14} /></>}
          </button>
        </section>
